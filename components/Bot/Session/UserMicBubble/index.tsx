...
const AudioIndicatorBubble: React.FC = () => {
  ...
  return <div ref={volRef} className="absolute inset-0 z-0 rounded-full transition-transform duration-100 ease transform scale-0 opacity-50 bg-green-300" />;
};
...
export default function UserMicBubble({ active, muted, handleMute }: Props) {
  ...
  const cx = clsx(
    muted && active && "opacity-100 bg-red-500 animate-pulseText border-6 border-red-200 outline-6 outline-red-400",
    !active && "pointer-events-none cursor-not-allowed",
    canTalk && "opacity-100 bg-primary-500 border-6 border-primary-200 outline-6 outline-primary-400 outline-offset-4"
  );

  return (
    <div className="relative z-20 flex flex-col mx-auto pt-5 md:pt-0">
      <div className={`relative cursor-pointer w-24 h-24 rounded-full flex justify-center items-center mx-auto z-20 transition-all duration-500 ease opacity-50 border-6 border-primary-300 outline-6 outline-primary-300 ${cx}`} onClick={handleMute}>
        <div className="relative z-20 opacity-30 transition-opacity duration-500 ease leading-none">
          {!active ? <Pause size={42} /> : canTalk ? <Mic size={42} /> : <MicOff size={42} />}
        </div>
