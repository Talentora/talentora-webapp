...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(
      `${MERGE_BASE_URL}/activities/${params.id}?include_remote_data=true`,
      { headers }
    );
...
export async function PATCH(
  ...
  try {
    ...
    const response = await fetch(`${MERGE_BASE_URL}/activities/${params.id}`, {
      method: 'PATCH',
      headers,
...
export async function DELETE(
  ...
  try {
    const headers = await getMergeHeaders();
    const response = await fetch(
      `${MERGE_BASE_URL}/activities/${params.id}`,
      {
        method: 'DELETE',
        headers
      }
