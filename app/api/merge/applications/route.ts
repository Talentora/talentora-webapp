...
export async function GET(request: Request) {
  try {
    ...
    const queryParams = new URLSearchParams({
      ...(searchParams.get('candidate_id') && { candidate_id: searchParams.get('candidate_id')! }),
      ...
      ...(searchParams.get('job_id') && { job_id: searchParams.get('job_id')! }),
...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${MERGE_BASE_URL}/applications`, {
      method: 'POST',
      headers,
