...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(
      `${MERGE_BASE_URL}/scorecards/${params.id}?include_remote_data=true`,
      { headers }
    );
    ...
    return NextResponse.json(
      { error: 'Failed to fetch scorecard details' },
      { status: 500 }
    );
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    const response = await fetch(`${MERGE_BASE_URL}/scorecards/${params.id}`, {
      method: 'PATCH',
      headers,
