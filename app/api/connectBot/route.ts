...
export async function POST(request: Request) {
    const interviewConfig = request.headers.get('interview-config') ? JSON.parse(request.headers.get('interview-config') as string) : defaultConfig;

  try {
    const response = await axios.post('https://talentora-bot-1.fly.dev/', interviewConfig, {
      headers: {
