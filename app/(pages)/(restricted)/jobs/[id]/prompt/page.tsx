...
// Define a type for node data that makes criteria optional
type NodeData = {
  label: string;
  content: string;
  criteria?: string;
...
const JobPromptPage = () => {
  ...
  const loadSavedFlow = useCallback(async () => {
    ...
    try {
      ...
      if (data?.prompt_graph && 
          typeof data.prompt_graph === 'object' && 
          data.prompt_graph !== null &&
          'nodes' in data.prompt_graph && 
          Array.isArray(data.prompt_graph.nodes) && 
          data.prompt_graph.nodes.length > 0 &&
          'edges' in data.prompt_graph && 
        ...
        const processedNodes = data.prompt_graph.nodes.map((node: any) => ({
          id: node.id,
          type: node.type,
          ...
          data: {
            ...
            ...(node.type === 'question' ? { criteria: node.data.criteria } : {})
          }
        }));
        setNodes(processedNodes);
        // Map JSON edges to proper Edge objects
        const processedEdges = data.prompt_graph.edges.map((edge: any) => ({
          id: edge.id,
          source: edge.source,
          target: edge.target,
  ...
  const saveFlow = useCallback(async () => {
    ...
    try {
      // Prepare the data to save - convert to plain JSON format
  ...
  const autoLayoutNodes = useCallback(() => {
    ...
    const traverseGraph = (nodeId: string): void => {
    ...
    let currentX = startX;
    let lastType: 'input' | 'section' | 'question' | 'conclusion' | null = null;
    ...
    flowOrder.forEach(nodeId => {
      ...
      switch (node.type) {
        ...
        case 'section': // Section node
          ...
          let questionY = centerY - ((questionIds.length - 1) * questionSpacingY / 2);
            ...
            const questionIndex: number = newNodes.findIndex(n => n.id === questionId);
            if (questionIndex !== -1) {
              ...
              const qIndex: number = flowOrder.indexOf(questionId);
              if (qIndex > -1) {
      ...
      lastType = (node.type === 'input' || node.type === 'section' || node.type === 'question' || node.type === 'conclusion') 
        ? node.type 
        : null;
    });
