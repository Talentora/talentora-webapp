...
export default function RecruiterSignUp({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const verifyEmailToken = async () => {
      ...
      try {
        const { email: verifiedEmail } = await verifyToken(token, companyId)
  ...
  useEffect(() => {
    const checkUser = async () => {
      ...
      if (session) {
        ...
        try {
          // If we have a token and it's valid, add the user to recruiters table
    ...
    checkUser()
  }, [router, supabase, tokenVerified, email, companyId, token])
    ...
    <div className="flex min-h-screen flex-col items-center justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p className="mt-2 text-center text-sm text-gray-600">
          Sign up to continue with your company setup
        </p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white px-4 py-8 shadow sm:rounded-lg sm:px-10">
          ...
          <div className="mt-6">
            ...
            <div className="mt-6">
              <Link href={`/signin/recruiter/${companyId}/protected?token=${token}`}>
